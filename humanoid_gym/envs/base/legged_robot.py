...
from humanoid_gym.utils.terrain import Terrain
from humanoid_gym.utils.math import quat_apply_yaw, wrap_to_pi
from humanoid_gym.utils.helpers import class_to_dict
from humanoid_gym.envs.base_task import BaseTask
from humanoid_gym.envs.base.legged_robot_config import LeggedRobotCfg
from humanoid_gym.utils.keyboard_controller import KeyBoardController, KeyboardAction, Delta
...
class LeggedRobot(BaseTask):
    ...
    def __init__(
        ...
        Args:
            cfg (LeggedRobotCfg): Configuration for the environment.
        ...
        if not self.headless:
            self.set_camera_view(self.cfg.viewer.pos, self.cfg.viewer.lookat)
        self._init_buffers()
    ...
    def set_actuator_commands(self):
    ...
    def compute_observations(self):
        ...
        if self.cfg.terrain.measure_heights:
            ...
            heights = torch.clip(heights, -1, 1.0)
    ...
    def _init_buffers(self):
        ...
        # Torque readings of all joints
        if self.cfg.asset.enable_joint_force_sensors:
            dof_force_tensor = self.gym.acquire_dof_force_tensor(self.sim)
            self.gym.refresh_dof_force_tensor(self.sim)
            self.dof_forces = gymtorch.wrap_tensor(dof_force_tensor).view(self.num_envs, self.num_dof)

        self.rigid_body_pos = self.rigid_body_state.view(self.num_envs, self.num_bodies, 13)
